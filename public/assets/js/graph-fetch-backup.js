...
(() => {
    const DEFAULTS = {
        ...
        GVIZ_RANGE: 'A:Z',
        SHEET_NAME_TOKEN_DATE: '{date}', // support mapping patterns
        // Which column labels we care about (normalized keys)
        KEYS: {
          timestamp: ['timestamp', 'time'],
          water_temp: ['water temp (c)', 'water_temp_c', 'water_temp', 'water temp', 'water_temp(c)'],
          air_temp: ['temp udara', 'air temp', 'air_temp', 'temperature'],
          humidity: ['humidity', 'humid'],
          tss: ['tss (v)', 'tss', 'tss_v'],
          ph: ['pH'.toLowerCase(), 'ph'],
    ...
    }, DEFAULTS, window.CLIMBOX_CONFIG || {});  
    ...
    function todayDateString() {
      ...
      function getSheetNameFromMapping(mapping, explicit) {
        ...
        const ms = mapping.sheetName || '';
        if (ms.includes(cfg.SHEET_NAME_TOKEN_DATE)) {
          return ms.replace(cfg.SHEET_NAME_TOKEN_DATE, todayDateString());
        }
        // If mapping already looks like data_YYYY-MM-DD, use today's version
        if (/^data_\d{4}-\d{2}-\d{2}$/.test(ms)) {
          return `data_${todayDateString()}`;
        }
        return ms || `data_${todayDateString()}`;
      }
      ...
      async function fetchJsonText(url, opts) {
        ...
        // Parse GViz response text to JSON string then object
  ...
  // Convert GViz table -> array of objects (header: value)
  function tableToRows(table) {
    ...
    const out = table.rows.map(r => {
      ...
      for (let i = 0; i < headers.length; i++) {
        ...
        // prefer cell.v, fallback to formatted cell.f
  ...
  // find best key in row (case-insensitive variations)
  function pickField(row, candidates) {
    ...
    for (const cand of candidates) {
      ...
      const direct = Object.keys(row).find(k => normalizeKey(k) === norm);
